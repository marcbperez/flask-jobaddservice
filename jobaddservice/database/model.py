...
class Model(object):
    ...
    def get_permitted(cls, id, user):
        ...
        return cls.query.filter(
            cls.id == id, ((cls.public) | (cls.user_id == user_id))).first()
    ...
    def get_permitted_models(
        ...
        user_id = user.id if user else 0
        models = cls.query.filter((cls.public) | (cls.user_id == user_id))
        ...
        total = models.count()
        # Sort, paginate result and get items.
        items = models.order_by(sort_direction).paginate(
